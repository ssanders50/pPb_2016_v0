...
static const int ntrkbins = 11;
static const  double trkBins[]={120, 135, 150, 160, 185, 210, 230, 250, 270, 300, 330, 350};
...
private:
  ...
  int fillTracks(const edm::Event& iEvent, const edm::EventSetup& iSetup, int bin)
  {
	...
	qxtrk[iorder-2]->Fill(itTrack->pt(), itTrack->eta(), eff*(TMath::Cos(iorder*itTrack->phi())));
	qytrk[iorder-2]->Fill(itTrack->pt(), itTrack->eta(), eff*(TMath::Sin(iorder*itTrack->phi())));
      }
  ...
  
{
  ...
  int npt = nptbins;
  for(int iorder = 2; iorder<=4; iorder++) {
    qxtrk[iorder-2] = fs->make<TH2D>(Form("qxtrk%d",iorder),Form("qxtrk%d",iorder),npt,ptbins, netabinsDefault, etabinsDefault);
    qytrk[iorder-2] = fs->make<TH2D>(Form("qytrk%d",iorder),Form("qytrk%d",iorder),npt,ptbins, netabinsDefault, etabinsDefault);
  ...
  if(makeTree_) {
    ...
    tree->Branch("qxtrk2",  "TH2D",  &qxtrk[0], 128000, 0);
    tree->Branch("qytrk2",  "TH2D",  &qytrk[0], 128000, 0);
    tree->Branch("qxtrk3",  "TH2D",  &qxtrk[1], 128000, 0);
    tree->Branch("qytrk3",  "TH2D",  &qytrk[1], 128000, 0);
    tree->Branch("qxtrk4",  "TH2D",  &qxtrk[2], 128000, 0);
    tree->Branch("qytrk4",  "TH2D",  &qytrk[2], 128000, 0);
...
VNAnalyzer::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  ...
  for(int ian = 0; ian<nanals; ian++) {
    if(ian==N2) Fill_N( N2,  ibin, qxtrk[0], qytrk[0], qcnt, qx[HFp2], qy[HFp2], qx[HFm2], qy[HFm2], qx[trackmid2], qy[trackmid2], sumw[HFp2], sumw[HFm2], sumw[trackmid2]);
    if(ian==N3) Fill_N( N3,  ibin, qxtrk[1], qytrk[1], qcnt, qx[HFp3], qy[HFp3], qx[HFm3], qy[HFm3], qx[trackmid3], qy[trackmid3], sumw[HFp3], sumw[HFm3], sumw[trackmid3]);
    if(ian==N4) Fill_N( N4,  ibin, qxtrk[2], qytrk[2], qcnt, qx[HFp4], qy[HFp4], qx[HFm4], qy[HFm4], qx[trackmid4], qy[trackmid4], sumw[HFp4], sumw[HFm4], sumw[trackmid4]);
  }
