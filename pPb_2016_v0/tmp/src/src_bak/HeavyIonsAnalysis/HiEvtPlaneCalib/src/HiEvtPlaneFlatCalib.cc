...
HiEvtPlaneFlatCalib::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  ...
  if(trackCollection_.isValid()){
    for(reco::TrackCollection::const_iterator itTrack = trackCollection_->begin(); itTrack != trackCollection_->end(); itTrack++){
      ...
      // 	hpixelTrack->Fill(2);
      // }
      if ( !itTrack->quality(reco::TrackBase::highPurity) ) accepted = false;
      if ( itTrack->charge() == 0 ) accepted = false;
      if ( fabs(itTrack->eta()) > 2.4 ) accepted = false;
      bool bPix = false;
      int nHits = itTrack->numberOfValidHits();
      if ( itTrack->pt() < 2.4 and (nHits==3 or nHits==4 or nHits==5 or nHits==6) ) bPix = true;
      if ( not bPix ) {
	if ( nHits < 11 ) accepted = false;
	...
	if(fabs(track_eta)<=2.4 && track_charge!=0) {
	  for(int i = 0; i<NumEPNames; i++) {
		...
		hPhiCent[i][bin]->Fill(itTrack->phi());
