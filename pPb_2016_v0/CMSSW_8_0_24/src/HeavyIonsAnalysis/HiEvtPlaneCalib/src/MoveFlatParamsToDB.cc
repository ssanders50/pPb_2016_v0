...
// class declaration
//
static const int MaxEPAllowed = 200;  //determined by RPFlatParams.h
...
//   edm::Service<TFileService> fs;
  ...
  double res1[MaxEPAllowed][100];
  double eres1[MaxEPAllowed][100];
  double res2[MaxEPAllowed][50];
  double eres2[MaxEPAllowed][50];
  double res5[MaxEPAllowed][20];
  double eres5[MaxEPAllowed][20];
  double res10[MaxEPAllowed][10];
  double eres10[MaxEPAllowed][10];
  double res20[MaxEPAllowed][5];
  double eres20[MaxEPAllowed][5];
  double res25[MaxEPAllowed][4];
  double eres25[MaxEPAllowed][40];
  double res30[MaxEPAllowed][3];
  double eres30[MaxEPAllowed][3];
  double res40[MaxEPAllowed][2];
  double eres40[MaxEPAllowed][2];
  string rescorT;
  // ----------member data ---------------------------
};
...

{
  ...
  string fname = iConfig.getParameter<std::string>("inputTFile");
  inFile = new TFile(fname.data());
  rescorT = iConfig.getParameter<std::string>("rescorT");
  if(inFile->IsZombie()) {
  ...
  nRP = cnt;
  cout<<"nRP = "<<nRP<<endl;
  FILE * ftest = fopen(rescorT.data(),"r");
  if(ftest) {
...
bool MoveFlatParamsToDB::GetRescor(double bin, string epname, double * restmp, double * erestmp) {
  FILE * ftest;
  ftest = fopen(Form("%s/%s_%04.1f.dat",rescorT.data(),epname.data(),bin),"r");
  if(ftest) {
    cout<<Form("%s/%s_%04.1f.dat FOUND",rescorT.data(),epname.data(),bin)<<endl;
  } else {
    return false; 
  }
  int nbins = 0;
